...
var addStrings = function(num1, num2) {
    ...
    let i = num1.length - 1; // 2，指向num1尾部
    let j = num2.length - 1; // 1，指向num2尾部
    let extra = 0; // 进位 
    let res = []; // 结果数组

    while (i >= 0 || j >= 0 || extra !== 0) {
        let s = ( i >= 0 ? parseInt(num1[i]) : 0 ); // 6
        let t = ( j >= 0 ? parseInt(num2[j]) : 0 ); // 7
        let sum = s + t + extra; // 13
        extra = ( sum >= 10 ? 1 : 0); // 进位，留给下次用
        let remainder = sum % 10; // 3，计算当前位(求余)
        res.unshift(remainder); // 将数字转字符串插入res头部
        i--;
        j--;
    }
